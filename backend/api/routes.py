...
async def get_history(author_id: Optional[str] = None, db: Session = Depends(get_db)):
    """
    Returns a list of all unique chat sessions (ideas) for the history sidebar,
    filtered by author_id. Mandatory parameter for privacy.
    """
    if not author_id:
        return []
    ...
    # Base query for first message of each conversation
    query = db.query(
        ChatSession.conversation_id,
        func.min(ChatSession.timestamp).label("min_ts")
    ).filter(ChatSession.authorId == author_id)
        
    subquery = query.group_by(ChatSession.conversation_id).subquery()
...
async def get_conversation_details(conversation_id: str, db: Session = Depends(get_db)):
    """
    Returns the full details (report + QA) for a specific conversation.
    """
    state_model = db.query(AgentStateModel).filter(AgentStateModel.conversation_id == conversation_id).first()
    first_session = db.query(ChatSession).filter(ChatSession.conversation_id == conversation_id).order_by(ChatSession.timestamp.asc()).first()
